<x-public-layouts.app :seo="$seo" title="Jenis Dokumen">
    <section class="rounded-3xl bg-gradient-to-br from-indigo-50/60 via-white to-white p-6 sm:p-8">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h1 class="text-3xl font-semibold tracking-tight text-gray-900">Jenis Dokumen</h1>
                <p class="mt-2 text-sm leading-relaxed text-gray-600">
                    Telusuri jenis dokumen untuk memudahkan browsing repository berdasarkan format/publikasi.
                </p>
            </div>
    ...
    <section class="mt-8">
        <div class="grid gap-4 md:grid-cols-2">
            @forelse($documentTypes as $type)
                <a href="{{ route('public.document-types.show', $type) }}"
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div class="min-w-0">
                            ...
                            <div class="mt-1 text-sm text-gray-600">
                                {{ $type->published_documents_count ?? 0 }} dokumen terbit
                            </div>
                        </div>
