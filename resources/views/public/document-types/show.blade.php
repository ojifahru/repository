<x-public-layouts.app :seo="$seo" :title="$documentType->name">
    <nav class="mb-5 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-gray-600" aria-label="Breadcrumb">
        <a class="font-medium text-gray-700 hover:text-gray-900 hover:underline"
            href="{{ route('public.document-types.index') }}">Jenis Dokumen</a>
        <span class="text-gray-300" aria-hidden="true">/</span>
        <span class="min-w-0 truncate text-gray-900">{{ $documentType->name }}</span>
    </nav>

    <section class="rounded-3xl bg-gradient-to-br from-indigo-50/60 via-white to-white p-6 sm:p-8">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0">
                <h1 class="text-3xl font-semibold tracking-tight text-gray-900">{{ $documentType->name }}</h1>
                <p class="mt-2 text-sm leading-relaxed text-gray-600">
                    Daftar dokumen terbit untuk jenis dokumen ini.
                </p>
            </div>
            <a class="inline-flex items-center justify-center rounded-2xl bg-white/80 px-5 py-3 text-sm font-semibold text-gray-800 shadow-sm ring-1 ring-gray-900/5 transition hover:bg-white focus:outline-none focus:ring-4 focus:ring-indigo-500/15"
                href="{{ route('public.documents.index', ['document_type_id' => $documentType->id]) }}">
                Filter dokumen
            </a>
        </div>
    </section>
    ...
    <section class="mt-10">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Daftar dokumen published untuk jenis ini.</p>
            </div>
        </div>
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2">
            ...
            @empty
                <div class="rounded-2xl bg-white/80 p-8 text-sm text-gray-600 shadow-sm ring-1 ring-gray-900/5">
                    Belum ada dokumen published untuk jenis dokumen ini.
                </div>
            @endforelse
        </div>
